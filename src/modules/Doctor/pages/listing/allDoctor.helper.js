// ./src/modules/Patient/listing.helper.js
export const listingHelper = {
  exportToCSV: (patients) => {
    const headers = ["ID", "Name", "Age", "Gender", "Phone", "Email", "Blood Group", "Admission Date", "Status"];
    const csvContent = [
      headers.join(","),
      ...patients.map((patient) =>
        [
          ...
          patient.email,
          patient.bloodGroup,
          patient.admissionDate, // Changed from registrationDate
          patient.status,
        ].join(","),
      ),
    ].join("\n");
  ...
  sortPatients: (patients, sortBy) => {
    return [...patients].sort((a, b) => {
      switch (sortBy) {
        ...
        case "date":
        default:
          return new Date(b.admissionDate) - new Date(a.admissionDate); // Changed from registrationDate
      }
    });
  },
};